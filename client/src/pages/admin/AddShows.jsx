...
const AddShows = () => {
  ...
  const fetchNowPlayingMovies = async () => {
    try {
      ...
      console.log('Error to fetching movies : ', error);
    }
  };

  const handleDateTimeAdd = () => {
    if (!dateTimeInput) return;
    const [date, time] = dateTimeInput.split("T");// date time string aa formate ma hase  "2025-07-04T18:30 so date and time ne saperate karva mate T thi split karva ma ave chhe.
    if (!date || !time) return;//If splitting didn’t produce both parts (malformed input), exit early.
